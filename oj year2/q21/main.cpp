...
public:
    ...
    */
    bool isForest()
    {
        ...
        {
            if (visited.find((*vertex)[i].getData()) == visited.end())
            {
                if (hasCycleDFS((*vertex)[i].getData(), visited, nullptr))
                {
                    return false;
                }
            }
...
private:
    bool hasCycleDFS(GraphNode<T> *current,
                     std::unordered_set<GraphNode<T> *> &visited,
                     GraphNode<T> *parent)
    {
        visited.insert(current);

        for (ListNode<GraphNode<T> *> *neighbor = current->getList()->getHead();
                neighbor != nullptr; neighbor = neighbor->getNext())
        {
            GraphNode<T> *nextNode = neighbor->getData();
            if (visited.find(nextNode) == visited.end())
            {
                if (hasCycleDFS(nextNode, visited, current))
                {
                    return true;
                }
            }
