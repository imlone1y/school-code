...
public:
    ...
    WeightedGraph *shortestPathTree(WeightedGraphVertex<V, E> *v)
    {
        ...
        std::set<std::pair<int, WeightedGraphVertex<V, E> *>> vertices;

        for (ListNode<WeightedGraphVertex<V, E> *> *cur = vertex->getHead();
                cur != nullptr; cur = cur->getNext())
        {
            WeightedGraphVertex<V, E> *vertex = cur->getData();
            vertex->setData(CHAR_MAX);
            vertices.insert({INT_MAX, vertex});
        }
        v->setData(0);
        vertices.insert({0, v});
        ...
        {
            WeightedGraphVertex<V, E> *u = vertices.begin()->second;
            vertices.erase(vertices.begin());

            WeightedGraphVertex<V, E> *newVertex = tree->addVertex(u->getData());
            if (u->getData() != v->getData())
            {
                WeightedGraphEdge<V, E> *edge =
                    new WeightedGraphEdge<V, E>(u->getData());
                tree->addLink(newVertex, (*u)[0]->getData()->getAnotherEnd(u),
                              edge->getData());
            }

            for (ListNode<WeightedGraphEdge<V, E> *> *cur = (*u)[0]; cur != nullptr;
                    cur = cur->getNext())
            {
                WeightedGraphVertex<V, E> *neighbor = cur->getData()->getAnotherEnd(u);
                int alt = u->getData() + cur->getData()->getData();
                if (alt < neighbor->getData())
                {
                    vertices.erase({neighbor->getData(), neighbor});
                    neighbor->setData(alt);
                    vertices.insert({alt, neighbor});
                }
            }
        }
